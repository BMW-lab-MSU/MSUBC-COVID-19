...
Original file is located at
    https://colab.research.google.com/drive/1vlH8kX7u3VJQZK3aUEgovTAut7OrUE5m
"""
...
def shape_data(csse_data, stock_data, weather_data, days_previous, start_date, end_date): #More datasources could be added here i.e (weather, historical search data, scraping, etc.) :: You need to work a bit on condencing this section since it was rushed
  ...
  weather_data2 = weather_data2.drop(weather_data2.columns[:1],axis=1)
  stock_data = stock_data.loc[:,  start_date:end_date]
  stock_data2 = stock_data.copy()
  stock_data2 = stock_data2.drop(stock_data2.columns[:1],axis=1)
  ...
  column_names2 = [f'weather_{number}_days_ago' for number in reversed(range(1,days_previous+1))]
  column_names3 = [f'stock_{number}_days_ago' for number in reversed(range(1,days_previous+1))]

  all_names = column_names3 + column_names2 + column_names
  ...
  for county in range(len(csse_data)):
    for days in range(len(csse_data.columns)-(7+days_previous)):
      Shaped_df.iloc[count] = pd.concat([stock_data2.iloc[county, 0+days:(days_previous)+days],weather_data2.iloc[county, 0+days:(days_previous)+days],csse_data.iloc[county, 0+days:(7+days_previous)+days]],axis=0)
      count += 1
  #print(Shaped_df)
...
training_days = 14   
unique_features = 3                      #Current Features: Weather and Case Counts  
...
shaped_df = shape_data(imported_df[0],imported_df[1],imported_df[2], training_days,start_date,end_date)
...
train_test_data.iloc[-7-days_of_the_week]

list(train_test_data[3][f'day_{days_of_the_week+1}_label'])
